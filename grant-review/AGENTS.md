...
The SSO callback stores the Hub-issued application count, signed logout URL, and short-lived encrypted actor token in the local session. When periodic Hub session revalidation expires, the shared client middleware must preserve the requested named-route URL in `url.intended` before invalidating the local session so SSO returns users to deep links rather than their role dashboard. “All applications” appears only for users assigned to multiple enabled Hub apps and returns to the launcher without ending either session. “Sign out” destroys the Grant Review session and follows the trusted signed Hub logout URL. The Hub then sends the browser through each registered front-channel endpoint; `/auth/hub/logout` validates the opaque transaction with the Hub before clearing Grant Review, so signing out anywhere immediately clears all application and Hub sessions before showing the contextual login screen. Never construct or accept an arbitrary logout URL.
...
Reviewer workflow: admins first invite active reviewers to declare COI for a round (`/apps/grant-review/admin/review-invitations`, labeled "COI invitations", table `reviewer_round_invitations`), which emails a conflict-of-interest invitation. Reviewers declare per-proposal responses (`conflict_of_interest_responses`, one explicit `clear` or `potential_conflict` row per screened proposal) before any assignment; each resubmission supersedes the previous declaration version (history preserved via `superseded_at`). Declarations are matched by reviewer+round, so legacy declarations without invitation linkage still count as coverage. COI status is visible at `/apps/grant-review/admin/conflicts` (including awaiting-declaration invitations) with an "Assign reviews" shortcut, and as per-proposal badges on the assignment page. Reported conflicts are advisory: the server only requires an active invitation for newly assigned reviewers; assigning a reviewer with a reported conflict triggers a confirmation dialog, and the decision remains the administrator's. Assignment of uninvited reviewers is rejected server-side. COI submissions notify all active Grant Review administrators through the configured mailer (delivery recorded in `admin_notified_at`; reviewer messaging only claims notification when delivery succeeded); the default UH deployment uses `post-office.uh.edu:25` from `donotreply@uh.edu`. Backfill legacy data with `php artisan reviewer-screening:backfill` (run `--dry-run` first); the legacy `conflict_of_interest_entries` table is retained read-only during the transition.
