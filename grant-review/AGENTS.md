...
Reviewer workflow: admins first invite active reviewers to screen a round (`/apps/grant-review/admin/review-invitations`, table `reviewer_round_invitations`), which emails a conflict-of-interest screening invitation. Reviewers declare per-proposal responses (`conflict_of_interest_responses`, one explicit `clear` or `potential_conflict` row per screened proposal) before any assignment; each resubmission supersedes the previous declaration version (history preserved via `superseded_at`). Declarations are matched by reviewer+round, so legacy declarations without invitation linkage still count as coverage. COI screening status is visible at `/apps/grant-review/admin/conflicts` (including awaiting-declaration invitations) with an "Assign reviews" shortcut, and as per-proposal badges on the assignment page. Reported conflicts are advisory: the server only requires an active invitation for newly assigned reviewers; assigning a reviewer with a reported conflict triggers a confirmation dialog, and the decision remains the administrator's. Assignment of uninvited reviewers is rejected server-side. COI submissions notify all active Grant Review administrators through the configured mailer (delivery recorded in `admin_notified_at`; reviewer messaging only claims notification when delivery succeeded); the default UH deployment uses `post-office.uh.edu:25` from `donotreply@uh.edu`. Backfill legacy data with `php artisan reviewer-screening:backfill` (run `--dry-run` first); the legacy `conflict_of_interest_entries` table is retained read-only during the transition.
